...
export function GrammarBindingDialog({ tag, onClose, onPublished }: GrammarBindingDialogProps) {
    ...
    setForm(
      existing === undefined
        ...
        : {
            ...
            minted: existing.scheme !== undefined,
  ...
  const crumbs: { label: string; go: Path }[] = [{ label: t("grammar.crumbRoot"), go: { at: "root" } }];
        ...
        <div className="mt-4 flex items-center justify-between gap-3 border-t pt-3">
          {existing !== undefined ? (
            <button
              ...
              onClick={() => {
                if (path.at === "posForm") {
                  setDraft(removePos(draft, path.value, existing.scheme));
                  setPath({ at: "pos" });
                } else if (path.at === "valueForm") {
                  setDraft(removeValue(draft, path.feature, path.value, existing.scheme));
                  setPath({ at: "values", feature: path.feature });
                }
              }}
    ...
    >
      <div className="flex max-h-[calc(100dvh-2rem)] w-full flex-col overflow-hidden rounded-t-xl border bg-surface shadow-lg sm:max-w-2xl sm:rounded-xl">
        <header className="border-b bg-surface-muted/60 px-4 py-3 sm:px-5">
          ...
          {/* One tab today. The strip exists so layer 2 slots in beside it
              rather than forcing a redesign. */}
          <div className="mt-3 flex gap-2" role="tablist">
            <span
              role="tab"
              aria-selected="true"
              className="rounded-full border border-primary bg-surface px-3 py-1 text-xs font-medium text-primary"
            >
